...
#define grf_writel(v, offset)	do { writel(v, (void*)RKIO_GRF_PHYS + offset); } while (0)

#define pmu_readl(offset)	readl((void*)RKIO_PMU_PHYS + offset)
#define pmu_writel(v, offset)	do { writel(v, (void*)RKIO_PMU_PHYS + offset); } while (0)
...
static int i2c_read(void *reg_addr, I2cSeg segment, int retry_count)
{
	...
	writel(I2C_MRXADDR(1, segment.chip << 1 | 1), reg_addr + I2C_MRXADDR_OFF);
	...
	while(bytes_remaining_to_be_transfered){
		if(bytes_remaining_to_be_transfered > 32){
			...
			bytes_to_be_transfered = bytes_remaining_to_be_transfered;
		}
